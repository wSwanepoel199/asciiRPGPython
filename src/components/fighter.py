...
class Fighter(BaseComponent):
    ...
    def __init__(self, Base_HP: int, Base_ATK: Tuple[int, int], Base_DEF: int, Base_ACC: int):
    ...
    @property
    def ATK(self) -> int:
        return [self.Base_ATK[0] + self.ATK_Bonus//2, self.Base_ATK[1] + self.ATK_Bonus]

    @property
    def ACC(self) -> int:
        return min(self.Base_ACC + self.ACC_Bonus, 100)
    ...
    @property
    def ACC_Bonus(self) -> int:
        if self.parent.equipment:
            return self.parent.equipment.acc_bonus
        else:
            return 0
    ...
    def die(self) -> None:
        if self.engine.player is self.parent:
            ...
            if self.engine.player.target == self.parent:
                self.engine.player.target = None
        ...
        self.engine.message_log.add_message(
            text=death_message, fg=death_message_colour)
