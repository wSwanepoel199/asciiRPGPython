...
class BaseAi(Action):
    ...
    def get_path_to(self, dest_x: int, dest_y: int) -> List[Tuple[int, int]]:
        ...
        cost = np.array(
            object=self.entity.gamemap.tiles["walkable"], dtype=np.int8)
...
class HostileAi(BaseAi):
    ...
    def perform(self) -> None:
        ...
        if self.engine.game_map.visible[self.entity.x, self.entity.y]:
            ...
            if self.entity.enemy_type:
                self.entity.enemy_type.action(entity=self.entity)
        ...
        if self.engine.game_map.visible[self.entity.x, self.entity.y]:
            ...
            if distance > 1 and distance < 5:
                self.path = self.get_path_to(
                    dest_x=self.last_seen[0],
                    dest_y=self.last_seen[1]
                )
        elif self.memory > 0 and not (
            self.entity.x == self.last_seen[0]
            and self.entity.y == self.last_seen[1]
        ):
            self.path = self.get_path_to(
                dest_x=self.last_seen[0],
                dest_y=self.last_seen[1]
