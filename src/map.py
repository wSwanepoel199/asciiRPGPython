...
class Map:
    ...
    def overworld(self, player, tile) -> bool:
        ...
        print("  HP - " + str(object=player["HP"]
                              ) + "/" + str(object=player["MAX_HP"]))
    ...
    def town(self, player) -> None:
        ...
        while player.location == "TOWN":
            ...
            print("  Name - " + player["name"])
            print(
                "  HP - " + str(object=player["HP"]) + "/" + str(object=player["MAX_HP"]))
            print("  ATK - " + str(object=player["ATK"]))
            print("  Potion(s) - " + str(object=player["potions"]))
            print("  Elixir(s) - " + str(object=player["elixirs"]))
            print("  Coin(s) - " + str(object=player["money"]))
    ...
    def mayor(self, player):
        ...
        while player.location == "MAYOR":
            ...
            if player.ATK < 10:
                print(
                    "You don't look strong enough to face the dragon! Come back once you've grown stronger!")
            else:
...
class GameMap:
    def __init__(
        ...
        self.tiles = np.full(shape=(self.width, self.height),
                             fill_value=self.tile_types["mapfill"], order="F")
        self.visible = np.full(
            shape=(self.width, self.height), fill_value=False, order="F")
    ...
    def modifyWall(self, x: int, y: int, dungeon: GameMap, wall_layout: list) -> Tuple[int, int, str]:
...
class GameWorld:
    ...
    def __init__(
        ...
        *,
        ...
        print(
            f"game world viewport width: {viewport_width}, game world viewport height: {viewport_height}")
